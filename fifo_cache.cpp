...
public:
    ...
    bool remove(const std::string& key) {
        ...
        // Remove from DB first
        {
            ...
            auto it = db.find(key);
            if (it != db.end()) {
                current_size -= (key.size() + it->second.size());
                db.erase(it);
    ...
    void insertToCache(const std::string& key, const std::string& value) {
        ...
        if(cache.find(key) != cache.end()){
            current_size -= (key.size() + cache[key].size()); //remove both footprint and itself
        }
...
int main() {
    ...
    // Add items
    fifo_cache.put("a", "Image1");
    fifo_cache.put("b", "Image2");
    fifo_cache.put("c", "Image3");

    //---- Put, delete, get tests ----------
    std::cout<< "---- Put, delete, get tests ----------" << std::endl;
    fifo_cache.put("d", "Image4"); // should evict a from queue and retrieve from db
    fifo_cache.displayCache();
    std::cout << "Access a: " << fifo_cache.get("a").second << std::endl; // Output: Image1 (from DB)
